...
export const formatPubkey = (pubkey) => {
    try {
        if (!validatePublicKey(pubkey)) {
            throw new NostrError('Invalid public key', NostrErrorCode.INVALID_KEY);
        }
        return nip19.npubEncode(pubkey);
    }
    catch (error) {
        logger.error('Failed to format public key', { error });
        throw new NostrError('Failed to format public key', NostrErrorCode.FORMATTING_ERROR, { cause: error });
    }
};
/**
