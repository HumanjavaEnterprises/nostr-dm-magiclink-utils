...
export class MagicLinkManager implements MagicLinkServiceInterface {
   ...
   */
  private readonly consumedTokens: ConsumedTokenStore;
  ...
  constructor(
    ...
    this.consumedTokens = consumedTokenStore || new InMemoryConsumedTokenStore();
  }
  ...
  async sendMagicLink(options: SendMagicLinkOptions): Promise<MagicLinkResponse> {
    try {
      ...
      if (
        ...
        throw new Error(
          'Invalid recipient public key: expected 64-character hex string'
        );
      }
   ...
   */
  private async cleanupConsumedTokens(): Promise<void> {
